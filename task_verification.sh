...
if [ -z "$TEST_ARGS" ]; then
  ...
  echo "Usage: ./verify.sh file1.test.js::TestName1,file2.test.js::TestName2"
...
try:
    ...
    for test in data['tests']:
        ...
        if ' | ' not in test['name']:
            raise ValueError('Test name must include file and test case (e.g., file.test.js | test name)')
